...
backcolorb=150;
backcolor=150;
color = {'spring','summer','autumn','winter','vga','cool','bone'};   % ´æ´¢Îªcell½á¹¹
...
ColorSection=zeros(m,n,3);
mask(indexl1)=1;
mask(indexl2)=1;
masks=repmat(mask,[1 1 3]);
ColorSection(masks)=yuvColor(masks);


[block_sizem,block_sizen]=size(Depth);

%% running AR model
[time,result]=AReq(block_sizem,block_sizen,SampleDepth,mask,halfwindowsize,Lamda,usespatial,halfsearchsize,sigmaw,usecolor,Color,sigmac,sigmab,usedepth,GuideDepth,sigmad);
indexout=result<0;
result(indexout)=0;
indexout=result>255;
result(indexout)=255;
