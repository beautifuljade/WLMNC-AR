 ...
 *    a : vector 1xnn
 *    b : vector 1xnn
 * output: for i=1:nn; res=res+(x(:,a(i))-y(:,b(i)))*(x(:,a(i))-y(:,b(i)))';end;
 * 
 * =============================================================
 */
                 ...
                 int nrhs, const mxArray *prhs[])
{
  ...
  if (nrhs != 4) {
    mexErrMsgTxt("Exactly for input arguments required.");
  } 
  ...
  n = mxGetN(prhs[0]);//cols
  m = mxGetM(prhs[0]);//rows
  mb = mxGetM(prhs[1]);
  if(m != mb)
    mexErrMsgTxt("Hey Bongo! Both input vectors must have same rows!\n");
  /* Get the data. */
  X  = mxGetPr(prhs[0]);
  Y  = mxGetPr(prhs[1]);
  av  = mxGetPr(prhs[2]);
